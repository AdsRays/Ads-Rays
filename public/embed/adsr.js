(function(){
  ...
  if(apiUrl){
    // Используем proxy endpoint для получения кампаний
    var proxyUrl = apiUrl + '/api/proxy/campaigns';
    safeFetchJSON(proxyUrl).then(function(list){ render(Array.isArray(list)?list:[]); });
  } else {
