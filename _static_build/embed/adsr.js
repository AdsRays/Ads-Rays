(function () {
  ...
  function skeleton() {
    ...
    for (var i = 1; i <= 3; i++) {
      ...
      card.style.cssText = "background:#162033;border-radius:12px;padding:12px;margin:10px 0;min-height:56px;display:flex;align-items:center;opacity:.7";
  ...
  function render(data) {
    ...
    (data.slice(0,3).length ? data.slice(0,3) : [1,2,3]).forEach(function (item, idx) {
      var card = el("div", "adsr-card");
      card.style.cssText = "background:#162033;border-radius:12px;padding:12px;margin:10px 0;min-height:56px;display:flex;align-items:center";
