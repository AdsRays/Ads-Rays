...
function install(prefix: string){
  ...
  app.get(`${prefix}/recommendations`, (_req,res)=>res.json([{ id:1, text:"Повысить бюджет на 10%" }]));
...
/** GET-зеркало для /api/report/pdf: нужно для прямой ссылки из фронта */
...
/** Общий обработчик ответа PDF (GET/POST) */
