    ...
    <div className="max-w-6xl mx-auto p-4 space-y-6">
      ...
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {overview?.trafficLight?.map((item: any) => (
            <TrafficCard 
              key={item.id}
              title={item.label} 
              color={item.color === 'red' ? 'bg-red-100' : item.color === 'yellow' ? 'bg-yellow-100' : 'bg-green-100'} 
              value={item} 
            />
          ))}
        </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {creatives.map(c => (
              <div key={c.id} className="border rounded p-3">
                <div className="flex justify-between"><span>Креатив {c.id}</span><span>{c.badge}</span></div>
                <div className="text-sm text-gray-600">CTR: {c.ctr}% • CPA: ${c.cpa}</div>
                <div className="text-xs text-gray-500 mt-1">{c.verdict}</div>
              </div>
            ))}
          </div>
        </div>
...
function TrafficCard({ title, color, value }: { title: string; color: string; value?: any }) {
  ...
  return (
    <div className={`p-4 rounded ${color}`}>
      ...
      <div className="text-sm text-gray-700">{value.note}</div>
    </div>
  );
}
